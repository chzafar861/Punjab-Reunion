...
export function ProfileCard({ profile, index = 0 }: { profile: Profile; index?: number }) {
  ...
  {/* Elderly man portrait placeholder */}
  ...
  return (
    <motion.div 
      ...
      className="group relative bg-card rounded-xl overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border border-border"
    >
      ...
      <div className="p-6">
        ...
        <Link href={`/profile/${profile.id}`}>
          <div className="inline-flex items-center gap-2 text-primary hover:text-primary/80 font-semibold text-sm cursor-pointer">
